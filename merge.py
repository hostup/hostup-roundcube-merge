...
def merge_databases(config_db1, config_db2):
    ...
    # Merge Users, Contacts, and Identities
...
def merge_users(db1_cursor, db2_cursor, db2_cnx):
    ...
    for user in users:
        ...
        if row:
            ...
            db2_cursor.execute(
                "INSERT INTO users (username, created, last_login, failed_login, failed_login_counter, language, preferences) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                (user['username'], user['created'], user['last_login'], user['failed_login'], user['failed_login_counter'], user['language'], user['preferences']))
